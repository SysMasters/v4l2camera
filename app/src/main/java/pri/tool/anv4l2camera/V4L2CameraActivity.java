...
//                    buffer, // Y 像素数据
//                    width, // Y 行跨度
//                    buffer, // U 像素数据（假设像素格式是 I420）
//                    width / 2, // U 行跨度
//                    buffer, // V 像素数据（假设像素格式是 I420）
//                    width / 2, // V 行跨度
//                    () -> {
//                    } // 释放资源的回调，可以为空
//            );
...
//            NV21Buffer buffer = new NV21Buffer(data, width, height, null);
//            VideoFrame videoFrame = new VideoFrame(buffer, 0, timestampNS);
//            videoSource.getCapturerObserver().onFrameCaptured(videoFrame);
//            videoFrame.release();
//            VideoFrame videoFrame = new VideoFrame(i420Buffer, 0, 0);
        }
    }
...
//                boolean isExpand = (boolean) map.get("isExpanded");   //这种是通过tag传值
        ...
        //创建并设置适配器
        adapter = new MyExpandableListAdapter(pixformats, resolutions, V4L2CameraActivity.this, ivGoToChildClickListener);
        ...
        final AlertDialog dialog = new AlertDialog.Builder(this).setTitle("选择图像格式和分辨率").setView(linearLayoutMain)//在这里把写好的这个listview的布局加载dialog中
